...
<Note>
  **Per-sample streams** = the second-by-second arrays behind a single activity
  (heart rate, power, speed, cadence, altitude, distance, time). For Strava
  these are served on demand via a live passthrough — see
  [Activity Streams](/api-reference/guides/activity-streams). They are **not**
  stored; the summary metrics above are.
</Note>
...
<Accordion title="Strava">
  **Integration Method:** OAuth 2.0 + Webhooks
  ...
  - Per-sample activity streams (heart rate, power, speed, cadence, altitude, distance, time) available on demand via a live passthrough endpoint — fetched from Strava per request, not stored
