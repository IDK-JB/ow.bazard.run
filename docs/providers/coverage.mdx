...
<Accordion title="Strava">
  **Integration Method:** OAuth 2.0 + Webhooks
  ...
  - Per-sample activity streams (heart rate, power, speed, cadence, altitude, distance, time) also available on demand via a live passthrough endpoint — fetched from Strava per request, not stored (Bazard fork)
