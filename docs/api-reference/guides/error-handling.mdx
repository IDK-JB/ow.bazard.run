...
**Solution:** Some operations (like updating users) require logging in via the developer portal. Use JWT authentication for these endpoints. Deleting a user accepts either a developer JWT or an admin API key (server-to-server GDPR erasure).
