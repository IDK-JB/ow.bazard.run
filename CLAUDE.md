...
| `base_webhook_handler.py`, `webhooks.py`, `strava_webhooks.py`, `oura_webhooks.py`, `deprecated_webhooks.py`, `polar/webhook_handler.py` | Support framework du flux Fitbit : hook `ensure_user_subscription`, `response_model=None` + signatures `dict \| Response` (le handler Fitbit répond au challenge de vérification) | Surface de PR #11 au-delà du handler lui-même — ce sont ces fichiers qui peuvent entrer en conflit à la prochaine sync upstream. |
| `docs/api-reference/guides/error-handling.mdx` | Mention de `ApiKeyDep` sur le delete user (adaptation BAZ-341) | Doc alignée sur le patch `users.py`. |
| `backend/app/api/routes/v1/timeseries.py`, `backend/app/services/timeseries_service.py`, `backend/app/repositories/data_point_series_repository.py`, `backend/app/schemas/enums/timeseries_resolution.py`, `backend/app/schemas/model_crud/activities/data_point_series.py`, `backend/app/schemas/utils/metadata.py` | Le param `resolution` (`raw\|1min\|5min\|15min\|1hour`) de `GET /timeseries` est désormais honoré : downsampling serveur par buckets UTC alignés, agrégation SUM/AVG/MAX selon le type de série (`get_aggregation_method`). `raw` (défaut) inchangé. | Ecrit dans le style upstream, **destiné à une PR upstream** (branche `feat/timeseries-resolution`, commit `feat: Honor resolution parameter in timeseries endpoint`). Tests : `tests/services/test_timeseries_resolution.py`, `tests/api/v1/test_timeseries.py`. |
