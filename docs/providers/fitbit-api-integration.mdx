...
```
    ...
    **Configuration details:**
    ...
    | `FITBIT_WEBHOOK_VERIFY_CODE` | Subscriber verification code from the Fitbit Developer portal (shown when registering a subscriber endpoint). Required for incoming webhooks: without it, the Fitbit verification challenge always receives a 404. | — |
    | `FITBIT_SUBSCRIBER_ID` | Optional subscriber ID, only needed when the Fitbit application declares more than one subscriber endpoint (passed as `subscriberId` on subscription creation). | — |
